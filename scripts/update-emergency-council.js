...
async function main() {
  ...
  if (adminWallet.address.toLowerCase() !== expectedAdmin.toLowerCase()) {
    throw new Error(
  ...
  // Get new Emergency Council address
  const newEmergencyCouncil = process.env.NEW_EMERGENCY_COUNCIL;
  if (!newEmergencyCouncil) {
    throw new Error(
      "NEW_EMERGENCY_COUNCIL environment variable is required.\n" +
      "Please add the new emergency council address to .env file:\n" +
      "NEW_EMERGENCY_COUNCIL=0xYourNewSecureAddressHere\n\n" +
      "⚠️  IMPORTANT: This should be a secure address you control (NOT the compromised deployer)"
    );
  }

  // Validate address format
  if (!ethers.isAddress(newEmergencyCouncil)) {
    throw new Error(`Invalid address format: ${newEmergencyCouncil}`);
  }

  console.log("\n📋 Configuration:");
  console.log("  Current Admin:", adminWallet.address);
  console.log("  New Emergency Council:", newEmergencyCouncil);
  ...
  if (currentEmergencyCouncil.toLowerCase() === compromisedDeployer.toLowerCase()) {
    ...
    console.log("  This needs to be updated immediately for security.");
  }

  // Confirm the change
  console.log("\n⚠️  IMPORTANT: Please verify this is correct!");
  console.log("  You are about to change the Emergency Council address.");
  console.log("  This action requires admin privileges.\n");

  // Execute the update
  console.log("🔄 Updating Emergency Council address...\n");

  try {
    const tx = await protocolGovernance.setEmergencyCouncil(newEmergencyCouncil);
    ...
    if (updatedEmergencyCouncil.toLowerCase() === newEmergencyCouncil.toLowerCase()) {
      console.log("\n✅ Emergency Council successfully updated!");

      // Update deployment file
      console.log("\n💾 Updating deployment-base-mainnet.json...");
      deploymentInfo.config.emergencyCouncil = newEmergencyCouncil;
      ...
      deploymentInfo.updates.push({
        ...
        action: "Emergency Council Updated",
        from: currentEmergencyCouncil,
        to: newEmergencyCouncil,
        txHash: tx.hash
      });
      ...
      console.log("\n✅ Emergency Council Address Updated Successfully!");
      ...
      console.log("\n🔒 Security Status:");
      console.log("  ✅ Emergency Council is now using a secure address");
      console.log("  ✅ Compromised deployer key can no longer use emergency functions");
      console.log("  ✅ All protocol controls are secure");

      console.log("\n📝 Next Steps:");
      console.log("  1. ✅ Verify the transaction on BaseScan");
      console.log("  2. ✅ Ensure you have secure backup of the new Emergency Council private key");
      console.log("  3. ✅ Consider using a multi-sig for Emergency Council in the future");
      console.log("  4. ✅ Revoke/delete the compromised deployer private key");
      console.log("  5. ✅ Update your security documentation");

    } else {
      console.log("\n❌ ERROR: Emergency Council update verification failed!");
      console.log(`  Expected: ${newEmergencyCouncil}`);
...
main()
  .then(() => {
    console.log("\n✅ Emergency Council update completed successfully!");
    process.exit(0);
  })
  .catch((error) => {
    console.error("\n❌ Emergency Council update failed:");
    console.error(error);
    process.exitCode = 1;
  });
